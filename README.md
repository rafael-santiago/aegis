...
``Aegis`` is a library that allows you detect if your software is being debugged or not on ``Linux``, ``FreeBSD`` and ``NetBSD``.
...
- To pass ``-lpthread`` flag if you are on ``Linux``, ``FreeBSD`` or ``NetBSD``.
